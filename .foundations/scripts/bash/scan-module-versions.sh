...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --org) ORG="$2"; shift 2 ;;
    --workspace) shift 2 ;;  # Accepted but unused: nothing filters by workspace yet.
...
scan_local_modules() {
  ...
  while IFS= read -r tf_file; do
    ...
    # shellcheck disable=SC2094 # "$tf_file" is only a --arg value to jq here; nothing writes to it
